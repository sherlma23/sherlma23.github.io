...
(function($){ 
    ...
    function intoSearch() {
        ...
        $(".super-search-fm").each(function() {
            var parent = $(this).closest(".s-search");
            var checked = parent.find('.hide-type-list input:radio:checked').first();
            syncSearchState(parent, checked);
        });
        toTarget($(".s-type-list.big"),false,false);
    }
    function syncSearchState(parent, checked) {
        if(!checked.length)
            return;
        var group = checked.parents(".search-group");
        var groupId = "";
        $.each((group.attr("class") || "").split(/\s+/), function(index, item) {
            if(item.indexOf("group-") === 0)
                groupId = item;
        });
        parent.find('.search-group').removeClass("s-current");
        group.addClass("s-current");
        parent.find('.s-type-list label').removeClass('active');
        var activeMenu = parent.find('.s-type-list label[data-id="'+groupId+'"]');
        activeMenu.addClass('active');
        if(activeMenu.length)
            window.localStorage.setItem("searchlistmenu", activeMenu.data("id"));
        parent.children(".super-search-fm").attr("action",checked.val());
        parent.find(".search-key").attr("placeholder",checked.data("placeholder"));
        if(checked.attr('id')=="type-zhannei" || checked.attr('id')=="m_type-zhannei")
    ...
    $(document).on('click', '.s-type-list label', function(event) {
        //event.preventDefault();
        var parent = $(this).closest(".s-search");
        var target = parent.find('#'+$(this).attr("for"));
        if(target.length){
            target.prop('checked', true);
            window.localStorage.setItem("searchlist", target.attr("id").replace("m_",""));
            syncSearchState(parent, target);
        }
